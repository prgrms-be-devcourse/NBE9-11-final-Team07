...
export function CouponsScreen() {
  ...
  useEffect(() => {
    ...
    couponApi.getMyCoupons()
      .then((response) => {
        if (!cancelled) setCoupons(response.filter((coupon) => coupon.status === 'ISSUED'))
      })
            ...
            {coupons.map((coupon) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[13px] font-semibold text-foreground line-clamp-1">{coupon.name}</p>
                ...
                <div className="text-right shrink-0 space-y-1">
                  <p className="text-[14px] font-black text-[oklch(0.62_0.24_25)]">{formatDiscount(coupon)}</p>
